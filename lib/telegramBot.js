...
bot.onText(new RegExp('^'+botCommands['report']+' (.*)$', 'i'), (msg, match) => {
    ...
    apiInterfaces.pool(apiRequest, function(error, stats) {    
        if (error || !stats) {
            log('error', logSystem, 'Unable to get API data for stats: ' + error);
            return bot.sendMessage(id, 'Unable to get pool statistics. Please retry.');
        }

        var minerHashrate = utils.getReadableHashRate(stats.hashrate);
        var minerHashes = utils.getReadableHashRate(stats.hashes);
        var minerPaid = utils.getReadableCoins(stats.paid);
        var minerBalance = utils.getReadableCoins(stats.balance);
