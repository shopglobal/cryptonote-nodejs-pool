    ...
    apiInterfaces.pool(apiRequest, function(error, data) {    
        ...
        if (data){
            ...
            if (data.stats.hashrate){
                minerHashrate = utils.getReadableHashRate(data.stats.hashrate);
            }
            if (data.stats.hashes){
                minerHashes = data.stats.hashes; 
            }
            if (data.stats.paid){
                minerPaid = utils.getReadableCoins(data.stats.paid);
            }
            if (data.stats.balance){
                minerBalance = utils.getReadableCoins(data.stats.balance);
            }     
        ...
        var response = '';
        response += '*Unable to get pool statistics. Please retry again later*\n';
        }
